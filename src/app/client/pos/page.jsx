...
export default function POSPage() {
  ...
  const fetchStores = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/client/stores`, {
        headers: getAuthHeaders()
      })
      const data = await response.json()
      console.log('📍 Stores fetched:', data.stores?.length)
      setStores(data.stores || [])
      if (data.stores?.length > 0) {
        setSelectedStore(data.stores[0])
        console.log('✅ Auto-selected store:', data.stores[0].name, data.stores[0].id)
      }
    } catch (error) {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const data = await response.json()
      console.log('🏷️ Categories fetched:', data.categories?.length)
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      console.log('📦 Fetching products for store:', selectedStore.id, 'category:', selectedCategory)
      
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/pos/products/category?${params}`, {
        headers: getAuthHeaders()
      })
  ...
  const fetchTodayStats = async () => {
    ...
    try {
      console.log('📊 Fetching today stats for store:', selectedStore.id)
      
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/pos/sales/today?store_id=${selectedStore.id}`, {
        headers: getAuthHeaders()
      })
      
      if (!response.ok) {
        console.error('❌ Today stats fetch failed:', response.status)
        return
      }
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      console.log('🔍 Searching:', query, 'in store:', selectedStore.id)
      
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/pos/products/search?${params}`, {
        headers: getAuthHeaders()
      })
      const data = await response.json()
      console.log('✅ Search results:', data.products?.length)
      setProducts(data.products || [])
    } catch (error) {
  ...
  const handlePayment = async (paymentMethod, customerInfo) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/pos/sales`, {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({
          store_id: selectedStore.id,
      ...
      const data = await response.json()
      console.log('✅ Payment successful:', data.receipt_number)

  ...
  const fetchAllReports = async () => {
    try {
      ...
      const [summaryRes, valueRes, turnoverRes, lowStockRes] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/inventory`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/inventory/stock-value`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/inventory/turnover?days=30`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/inventory/low-stock`, {
          headers: { 'Authorization': `Bearer ${token}` }
        })
      ]);
                        ...
                        >
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              <Badge variant={product.stock_status === 'out_of_stock' ? 'destructive' : 'warning'}>
